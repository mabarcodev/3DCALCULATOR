...
export function Breakdown({ acc, inputs, result, t, currency }: BreakdownProps) {
  ...
  return (
    <Block num="08" title={t.desglosTitle} right={<span style={{ color: acc }}>{fmt(result.subtotal)}</span>}>
      <div style={{ padding: 20 }}>
        ...
        <div style={{ fontFamily: MONO, borderTop: '2px solid #fafafa', marginTop: 16, paddingTop: 14, display: 'flex', justifyContent: 'space-between', gap: 12, fontSize: 14, fontWeight: 700, letterSpacing: '0.15em' }}>
          <span>{t.subtotalLabel}</span><span>{fmt(result.subtotal)}</span>
        </div>
        <div style={{ fontFamily: MONO, paddingTop: 8, display: 'flex', justifyContent: 'space-between', gap: 12, fontSize: 14, fontWeight: 700, letterSpacing: '0.15em', color: acc }}>
