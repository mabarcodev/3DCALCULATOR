...
export function FinalPrice({ acc, price, t, currency }: FinalPriceProps) {
  ...
  return (
    <section style={{ background: acc, color: '#0a0a0a', border: '2px solid #fafafa', position: 'relative', overflow: 'hidden', marginBottom: 16 }} aria-labelledby="final-price-title">
      <div style={{ fontFamily: MONO, display: 'flex', justifyContent: 'space-between', padding: '6px 14px', background: '#0a0a0a', color: acc, fontSize: 11, letterSpacing: '0.3em', fontWeight: 700 }}>
        <h2 id="final-price-title" style={{ margin: 0, font: 'inherit', letterSpacing: 'inherit' }}>{t.finalPriceLabel}</h2>
        <span>OUTPUT.{currency}</span>
      </div>
      <div style={{ padding: '24px 20px 20px' }}>
        <div className="final-price-value" style={{ fontFamily: DISPLAY, fontWeight: 900, lineHeight: 0.78, letterSpacing: 0, fontVariantNumeric: 'tabular-nums', marginBottom: 14 }}>
        ...
        <button type="button" onClick={copy}
          ...
          style={{ fontFamily: MONO, width: '100%', padding: 16, background: '#0a0a0a', color: acc, border: '2px solid #0a0a0a', fontSize: 14, letterSpacing: '0.2em', fontWeight: 700, cursor: 'pointer', textTransform: 'uppercase', transition: 'all 0.15s' }}>
