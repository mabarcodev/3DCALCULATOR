    ...
    <main className="app-shell">
      ...
      <Block num="01" title={t.materialTitle} right={<span style={{ color: acc }}>{currency}/KG</span>}>
        <div style={{ padding: 20, display: 'grid', gridTemplateColumns: '1fr auto', alignItems: 'center', gap: 16 }}>
          <span className="metric-value" style={{ fontFamily: DISPLAY, fontWeight: 900, lineHeight: 0.85, letterSpacing: 0, fontVariantNumeric: 'tabular-nums' }}>
            {fmt2(inputs.materialPricePerKg)}<span style={{ color: acc, fontSize: '0.5em' }}>{currency}</span>
          </span>
          <Stepper label="Precio del material por kilogramo" value={inputs.materialPricePerKg} step={0.5} onChange={(v) => update('materialPricePerKg', v)} />
        </div>
